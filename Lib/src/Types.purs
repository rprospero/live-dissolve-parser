...
xmlLayer :: (Array (Tuple String (Array LayerPart))) -> State XmlNode Unit
xmlLayer xs = for_ xs go
  where
  go (Tuple name ss) = onNewChild "layer" $ onAttr "name" name *> for_ ss xmlOnLayer
